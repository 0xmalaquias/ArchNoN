# [ anonproxy-automation ]
[!] Automation/Script of tor as a secure proxy on linux distros also a process manager in real time(about tor, service automation available only for systemd, runit, openrc)

[!] Actions: --start, --stop, --restart, --boot-enable, --boot-disable, --myip, --change, --fix, --set-bridges --restart-torrc, --help.

[!] Use: git clone https://github.com/NewMaster27/anonproxy && cd anonproxy && chmod +x make_bin.sh && sudo ./make_bin.sh && anonproxy help

[!] In addition: 
- Recommended browsers: falkon, surf, torbrowser, librewolf, mullvad, badwolf.
- Recommended: MAC address constant changing.
- Recommended: Use an aditional proxy: proton, mullvad, etc.
- Change iptables rules if needed, by default script modify this to make your traffic tor only, so: tor is killed=no wifi.

[+] Enjoy it !!
